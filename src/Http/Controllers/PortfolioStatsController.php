...
use Canvas\Models\PortfolioView;
use Canvas\Models\PortfolioVisit;
...
class PortfolioStatsController extends Controller
{
    ...
    public function index(): JsonResponse
    {
        ...
        $views = PortfolioView::query()
                     ->select('created_at')
                     ->whereIn('portfolio_id', $posts->pluck('id'))
    ...
    public function show(string $id): JsonResponse
    {
        ...
        $previousMonthlyViews = $portfolio->views->whereBetween('created_at', [
            today()->subMonth()->startOfMonth()->startOfDay()->toDateTimeString(),
            today()->subMonth()->endOfMonth()->endOfDay()->toDateTimeString(),
        ]);

        $currentMonthlyViews = $portfolio->views->whereBetween('created_at', [
            today()->startOfMonth()->startOfDay()->toDateTimeString(),
            today()->endOfMonth()->endOfDay()->toDateTimeString(),
        ]);

        $lastThirtyDays = $portfolio->views->whereBetween('created_at', [
            today()->subDays(30)->startOfDay()->toDateTimeString(),
            today()->endOfDay()->toDateTimeString(),
        ]);

        $previousMonthlyVisits = $portfolio->visits->whereBetween('created_at', [
            today()->subMonth()->startOfMonth()->startOfDay()->toDateTimeString(),
            today()->subMonth()->endOfMonth()->endOfDay()->toDateTimeString(),
        ]);

        $currentMonthlyVisits = $portfolio->visits->whereBetween('created_at', [
            today()->startOfMonth()->startOfDay()->toDateTimeString(),
            today()->endOfMonth()->endOfDay()->toDateTimeString(),
        ]);
        ...
        return response()->json([
            'portfolio' => $portfolio,
            'readTime' => $portfolio->read_time,
            'popularReadingTimes' => $portfolio->popular_reading_times,
            'topReferers' => $portfolio->top_referers,
            ...
            'traffic' => [
                ...
                'visits' => $this->calculateTotalForDays($portfolio->visits, 30)->toJson(),
            ],
        ]);
    }
