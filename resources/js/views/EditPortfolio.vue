<template>
  <section>
          ...
          <li class="text-muted font-weight-bold">
            <div class="border-left pl-3">
              <div v-if="!isSaving && !isSaved">
                <span v-if="isPublished(portfolio.published_at)">{{ trans.published }}</span>
                <span v-if="isDraft(portfolio.published_at)">{{ trans.draft }}</span>
              </div>
      ...
      <template slot="options">
        <div class="dropdown">
          ...
          <div class="dropdown-menu dropdown-menu-right">
            <div v-if="isPublished(portfolio.published_at)" class="dropdown-divider"/>
            <a
            ...
            <a v-if="!creatingPortfolio" href="#" class="dropdown-item text-danger" @click="showDeleteModal">
              {{ trans.delete }}
            </a>
          </div>
        </div>
    ...
    <main v-if="isReady" class="py-4">
      <div class="col-xl-8 offset-xl-2 col-lg-10 offset-lg-1 col-md-12">
        ...
        <div class="form-group my-2">
          <quill-editor :key="portfolio.id" :post="portfolio" :language-selected="languageSelected" @update-post="savePortfolio"/>
        </div>
      </div>
    </main>
    ...
    <section v-if="isReady">
      ...
      />
      <publish-modal ref="publishModal" :post="portfolio" @publish="updatePublishedAt"/>
...
export default {
  ...
  methods: {
    fetchPortfolio() {
      return this.request()
          ...
          .then(({data}) => {
            ...
            this.portfolio.info.project_name.it = get(data.portfolio.info, 'project_name.it', '');
            this.portfolio.info.project_name.en = get(data.portfolio.info, 'project_name.en', '');
            this.portfolio.info.project_name.es = get(data.portfolio.info, 'project_name.es', '');
            this.portfolio.info.project_name.fr = get(data.portfolio.info, 'project_name.fr', '');
            this.portfolio.info.project_name.de = get(data.portfolio.info, 'project_name.de', '');
            this.portfolio.info.location.it = get(data.portfolio.info, 'location.it', '');
            this.portfolio.info.location.en = get(data.portfolio.info, 'location.en', '');
            this.portfolio.info.location.es = get(data.portfolio.info, 'location.es', '');
            this.portfolio.info.location.fr = get(data.portfolio.info, 'location.fr', '');
            this.portfolio.info.location.de = get(data.portfolio.info, 'location.de', '');
            ...
            this.portfolio.featured_image = get(data.portfolio, 'featured_image', '');
            this.portfolio.featured_image_caption = get(data.portfolio, 'featured_image_caption', '');
            this.portfolio.gallery_images = get(data.portfolio, 'gallery_images', [])
            this.portfolio.meta.description.it = get(data.portfolio.meta, 'description.it', '');
            this.portfolio.meta.description.en = get(data.portfolio.meta, 'description.en', '');
            this.portfolio.meta.description.es = get(data.portfolio.meta, 'description.es', '');
            this.portfolio.meta.description.fr = get(data.portfolio.meta, 'description.fr', '');
            this.portfolio.meta.description.de = get(data.portfolio.meta, 'description.de', '');
            this.portfolio.meta.title.it = get(data.portfolio.meta, 'title.it', '');
            this.portfolio.meta.title.en = get(data.portfolio.meta, 'title.en', '');
            this.portfolio.meta.title.es = get(data.portfolio.meta, 'title.es', '');
            this.portfolio.meta.title.fr = get(data.portfolio.meta, 'title.fr', '');
            this.portfolio.meta.title.de = get(data.portfolio.meta, 'title.de', '');
            this.portfolio.meta.canonical_link.it = get(data.portfolio.meta, 'canonical_link.it', '');
            this.portfolio.meta.canonical_link.en = get(data.portfolio.meta, 'canonical_link.en', '');
            this.portfolio.meta.canonical_link.es = get(data.portfolio.meta, 'canonical_link.es', '');
            this.portfolio.meta.canonical_link.fr = get(data.portfolio.meta, 'canonical_link.fr', '');
            this.portfolio.meta.canonical_link.de = get(data.portfolio.meta, 'canonical_link.de', '');
            this.portfolio.portfolio_category = get(data.portfolio, 'portfolio_category', []);
    ...
    async savePortfolio() {
      ...
      this.isSaving = true;
      this.isSaved = false;
      this.portfolio.title[this.languageSelected] = this.portfolio.title[this.languageSelected] || 'Title';
      ...
      if (isEmpty(this.errors) && this.creatingPortfolio) {
        await this.$router.push({name: 'edit-portfolio', params: {id: this.portfolio.id}});
        NProgress.done();
      }
