          ...
          <div class="card-body">
            <div class="col-12">
              <div class="form-group row">
                ...
                <input
                    v-model="topic.name[languageSelected]"
                    type="text"
                    name="name"
                    autofocus
                    autocomplete="off"
...
export default {
  ...
  computed: {
    ...
    shouldDisableButton() {
      return isEmpty(this.topic.name.it);
    },

    title() {
      if (this.creatingTopic) {
        return this.topic.name.it || this.trans.new_topic;
      } else {
        return this.topic.name.it || this.trans.edit_topic;
      }
    },
    ...
    fetchTopic() {
      return this.request()
          ...
          .then(({data}) => {
            ...
            this.topic.name.it = get(data.name, 'it', '');
            this.topic.name.en = get(data.name, 'en', '');
            this.topic.name.es = get(data.name, 'es', '');
            this.topic.name.fr = get(data.name, 'fr', '');
            this.topic.name.de = get(data.name, 'de', '');
