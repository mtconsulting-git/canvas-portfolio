          ...
          <div class="card-body">
            <div class="col-12">
              <div class="form-group row">
                ...
                <input
                    v-model="tag.name[languageSelected]"
                    type="text"
                    name="name"
                    autofocus
                    autocomplete="off"
...
export default {
  ...
  computed: {
    ...
    shouldDisableButton() {
      return isEmpty(this.tag.name.it);
    },

    title() {
      if (this.creatingTag) {
        return this.tag.name.it || this.trans.new_tag;
      } else {
        return this.tag.name.it || this.trans.edit_tag;
      }
    },
  ...
  methods: {
    fetchTag() {
      return this.request()
          ...
          .then(({data}) => {
            this.tag.id = data.id;
            this.tag.name.it = get(data.name, 'it', '');
            this.tag.name.en = get(data.name, 'en', '');
            this.tag.name.es = get(data.name, 'es', '');
            this.tag.name.fr = get(data.name, 'fr', '');
            this.tag.name.de = get(data.name, 'de', '');
