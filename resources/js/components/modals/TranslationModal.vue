...
export default {
  ...
  methods: {
    async translate() {
      ...
      if (this.elem.title) { //post, portfolio
      ...
      } else if (this.elem.name) { //category, tag, topic
      ...
      await this.request()
          ...
          .then(({data}) => {
            if (data.title) { //post and portfolio
              ...
              if (this.elem.info) { //portfolio
                this.elem.info.project_name[this.languageSelected] = get(data.info, 'project_name');
                this.elem.info.location[this.languageSelected] = get(data.info, 'location');
              }

              this.elem.meta.description[this.languageSelected] = get(data.meta, 'description');
              this.elem.meta.title[this.languageSelected] = get(data.meta, 'title');
              this.elem.meta.canonical_link[this.languageSelected] = get(data.meta, 'canonical_link');

            } else if(data.name) { //category, tag, topic
